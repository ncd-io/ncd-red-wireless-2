		...
		'81': {
			...
			parse: (d) => {
				...
				return {
					...
					s1_odr: odr1,
					s1_temperature: signInt(d.slice(2, 4).reduce(msbLsb), 16) / 100,

					x1_rms_ACC_G: d.slice(4, 6).reduce(msbLsb)/1000,
					x1_max_ACC_G: d.slice(6, 8).reduce(msbLsb)/1000,
					x1_velocity_mm_sec: d.slice(8, 10).reduce(msbLsb) / 100,
					x1_displacement_mm: d.slice(10, 12).reduce(msbLsb) / 100,
					x1_peak_one_Hz: d.slice(12, 14).reduce(msbLsb),
					x1_peak_two_Hz: d.slice(14, 16).reduce(msbLsb),
					x1_peak_three_Hz: d.slice(16, 18).reduce(msbLsb),

					y1_rms_ACC_G: d.slice(18, 20).reduce(msbLsb)/1000,
					y1_max_ACC_G: d.slice(20, 22).reduce(msbLsb)/1000,
					y1_velocity_mm_sec: d.slice(22, 24).reduce(msbLsb) / 100,
					y1_displacement_mm: d.slice(24, 26).reduce(msbLsb) / 100,
					y1_peak_one_Hz: d.slice(26, 28).reduce(msbLsb),
					y1_peak_two_Hz: d.slice(28, 30).reduce(msbLsb),
					y1_peak_three_Hz: d.slice(30, 32).reduce(msbLsb),

					z1_rms_ACC_G: d.slice(32, 34).reduce(msbLsb)/1000,
					z1_max_ACC_G: d.slice(34, 36).reduce(msbLsb)/1000,
					z1_velocity_mm_sec: d.slice(36, 38).reduce(msbLsb) / 100,
					z1_displacement_mm: d.slice(38, 40).reduce(msbLsb) / 100,
					z1_peak_one_Hz: d.slice(40, 42).reduce(msbLsb),
					z1_peak_two_Hz: d.slice(42, 44).reduce(msbLsb),
					z1_peak_three_Hz: d.slice(44, 46).reduce(msbLsb),

					s2_odr: odr2,
					s2_temperature: signInt(d.slice(47, 49).reduce(msbLsb), 16) / 100,

					x2_rms_ACC_G: d.slice(4, 6).reduce(msbLsb)/1000,
					x2_max_ACC_G: d.slice(6, 8).reduce(msbLsb)/1000,
					x2_velocity_mm_sec: d.slice(8, 10).reduce(msbLsb) / 100,
					x2_displacement_mm: d.slice(10, 12).reduce(msbLsb) / 100,
					x2_peak_one_Hz: d.slice(12, 14).reduce(msbLsb),
					x2_peak_two_Hz: d.slice(14, 16).reduce(msbLsb),
					x2_peak_three_Hz: d.slice(16, 18).reduce(msbLsb),

					y2_rms_ACC_G: d.slice(18, 20).reduce(msbLsb)/1000,
					y2_max_ACC_G: d.slice(20, 22).reduce(msbLsb)/1000,
					y2_velocity_mm_sec: d.slice(22, 24).reduce(msbLsb) / 100,
					y2_displacement_mm: d.slice(24, 26).reduce(msbLsb) / 100,
					y2_peak_one_Hz: d.slice(26, 28).reduce(msbLsb),
					y2_peak_two_Hz: d.slice(28, 30).reduce(msbLsb),
					y2_peak_three_Hz: d.slice(30, 32).reduce(msbLsb),

					z2_rms_ACC_G: d.slice(32, 34).reduce(msbLsb)/1000,
					z2_max_ACC_G: d.slice(34, 36).reduce(msbLsb)/1000,
					z2_velocity_mm_sec: d.slice(36, 38).reduce(msbLsb) / 100,
					z2_displacement_mm: d.slice(38, 40).reduce(msbLsb) / 100,
					z2_peak_one_Hz: d.slice(40, 42).reduce(msbLsb),
					z2_peak_two_Hz: d.slice(42, 44).reduce(msbLsb),
					z2_peak_three_Hz: d.slice(44, 46).reduce(msbLsb),
				};
			}
		},
		'82': {
