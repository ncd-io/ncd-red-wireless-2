	...
	var types = {
		...
		'515': {
			...
			parse: (d) => {
				return {
					bank: d[1],
					rms_1: d.slice(2, 5).reduce(msbLsb),
					max_1: d.slice(5, 8).reduce(msbLsb),
					min_1: d.slice(8, 11).reduce(msbLsb),
					rms_2: d.slice(11, 14).reduce(msbLsb),
					max_2: d.slice(14, 17).reduce(msbLsb),
					min_2: d.slice(17, 20).reduce(msbLsb),
					rms_3: d.slice(20, 23).reduce(msbLsb),
					max_3: d.slice(23, 26).reduce(msbLsb),
					min_3: d.slice(26, 29).reduce(msbLsb),
					rms_4: d.slice(29, 32).reduce(msbLsb),
					max_4: d.slice(32, 35).reduce(msbLsb),
					min_4: d.slice(35, 38).reduce(msbLsb),
					rms_5: d.slice(38, 41).reduce(msbLsb),
					max_5: d.slice(41, 44).reduce(msbLsb),
					min_5: d.slice(44, 47).reduce(msbLsb),
					rms_6: d.slice(47, 50).reduce(msbLsb),
					max_6: d.slice(50, 53).reduce(msbLsb),
					min_6: d.slice(53, 56).reduce(msbLsb)
				}
			}
		},
