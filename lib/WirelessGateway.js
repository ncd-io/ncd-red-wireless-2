	...
	var types = {
		...
		'520': {
			...
			parse: (payload, parsed) => {
				return {
					current_1_ma: payload.slice(0, 3).reduce(msbLsb),
					frequency_1: payload.slice(3, 5).reduce(msbLsb),
					current_2_ma: payload.slice(5, 8).reduce(msbLsb),
					frequency_2: payload.slice(8, 10).reduce(msbLsb),
					current_3_ma: payload.slice(10, 13).reduce(msbLsb),
					frequency_3: payload.slice(13, 15).reduce(msbLsb),
					current_4_ma: payload.slice(15, 18).reduce(msbLsb),
					frequency_4: payload.slice(18, 20).reduce(msbLsb),
					current_5_ma: payload.slice(20, 23).reduce(msbLsb),
					frequency_5: payload.slice(23, 25).reduce(msbLsb),
					current_6_ma: payload.slice(25, 28).reduce(msbLsb),
					frequency_6: payload.slice(28, 30).reduce(msbLsb),
					temperature: signInt(payload.slice(30, 32).reduce(msbLsb), 16) * 100,
					humidity: payload.slice(32, 34).reduce(msbLsb) * 100
				}
			},
			'parse_fly': (frame) => {
				let frame_data = {};
