	...
	parse(frame){
		...
		if(typeof this[type] == 'function'){
			...
			if(mode === 'FLY' && frame.data.length > 12 && typeof this.sensor_types[data.sensor_type].parse_fly == 'function'){
				...
				// Add two  bytes to begginning to match the FLY parsing
				this.sensor_pool[frame.mac].reported_config = this.sensor_types[data.sensor_type].parse_fly([0,0].concat(frame.data));
			}
			var that = this;
