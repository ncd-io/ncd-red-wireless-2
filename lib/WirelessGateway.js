...
module.exports = class WirelessSensor{
	...
	parse(frame){
		...
		if(typeof this[type] == 'function'){
			...
			if(otf_devices.includes(device_type)){
				if(frame.data[9] == 70 && frame.data[10] == 76 && frame.data[11] == 89 && !this.hasOwnProperty('fly_101_in_progress')) {
			...
			if(mode == 'ACK'){
				if(frame.data[7] == 79 && frame.data[8] == 84 && frame.data[9] == 78 && !this.hasOwnProperty('fly_101_in_progress')){
				//	data.sensor_type = msbLsb(frame.data[2], frame.data[3]);
					data.sensor_type = (frame.data[3]);
					mode = "OTN";
				}
				else if(frame.data[7] == 79 && frame.data[8] == 84 && frame.data[9] == 70 && !this.hasOwnProperty('fly_101_in_progress')){
				//	data.sensor_type = msbLsb(frame.data[2], frame.data[3]);
					data.sensor_type = (frame.data[3]);
					mode = "OTF";
				}
	...
	sensor_data(payload, frame){
		...
		// #OTF
		var otf_devices = [80,81,82,101,102];
	...
	config_set_led_parameter_84(sensor_mac, value){
		console.log('config_set_led_parameter_84');
		var packet = [244, 79, 0, 0, 0, 40, value];
		console.log(packet);
		return this.config_send(sensor_mac, packet);
	}
	config_set_led_threshold_84(sensor_mac, value){
		console.log('config_set_led_threshold_84');
		var packet = [244, 79, 0, 0, 0, 40, value];
		console.log(packet);
		return this.config_send(sensor_mac, packet);
	}
	...
	var types = {
		...
		'75': {
			...
			parse: (d) => {
				return {
					adc: signInt(d.slice(0, 2).reduce(msbLsb), 16),
					mA: signInt(d.slice(2, 4).reduce(msbLsb), 16)/100,
					Velocity: (d.slice(4, 6).reduce(msbLsb), 16)/100
				};
			}
		},
		'76': {
		...
		'515': {
			...
			parse: (d, parsed, mac) => {
				let bank = d[9];
				let bank_total = d[8];
				if(!globalDevices.hasOwnProperty(mac) && bank == 1) {
					globalDevices[mac] = {};
					globalDevices[mac].data = {};
				}else if(!globalDevices.hasOwnProperty(mac) && bank != 1){
					console.log('packet error detected');
				}
				globalDevices[mac].data[bank] = {
					rms_1: d.slice(10, 13).reduce(msbLsb),
					max_1: d.slice(13, 16).reduce(msbLsb),
					min_1: d.slice(16, 19).reduce(msbLsb),
					rms_2: d.slice(19, 22).reduce(msbLsb),
					max_2: d.slice(22, 25).reduce(msbLsb),
					min_2: d.slice(25, 28).reduce(msbLsb),
					rms_3: d.slice(28, 31).reduce(msbLsb),
					max_3: d.slice(31, 34).reduce(msbLsb),
					min_3: d.slice(34, 37).reduce(msbLsb),
					rms_4: d.slice(37, 40).reduce(msbLsb),
					max_4: d.slice(40, 43).reduce(msbLsb),
					min_4: d.slice(43, 46).reduce(msbLsb),
					rms_5: d.slice(46, 49).reduce(msbLsb),
					max_5: d.slice(49, 52).reduce(msbLsb),
					min_5: d.slice(52, 55).reduce(msbLsb),
					rms_6: d.slice(55, 58).reduce(msbLsb),
					max_6: d.slice(58, 61).reduce(msbLsb),
					min_6: d.slice(61, 64).reduce(msbLsb)
				}
				if(bank == bank_total){
					let dev_data = globalDevices[mac];
