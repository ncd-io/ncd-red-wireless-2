...
module.exports = class WirelessSensor{
	...
	parse(frame){
		...
		if(typeof this[type] == 'function'){
			...
			}else if(mode === 'OTF' && frame.data.length > 12 && typeof this.sensor_types[device_type].parse_fly == 'function'){
				// Remove first two data units so we don't need to restructure the parse_fly to account for index offsets
				// OTF does not include the battery level which offsets everything by two bytes
				this.sensor_pool[frame.mac].reported_config = this.sensor_types[device_type].parse_fly(frame.data.slice(2));
			}
			var that = this;
